...
class Home extends React.Component {
    ...
    unlikeHandler = (postId, likeCount) => {
        let { instagramImageDetail, likedPostInfo } = this.state;
        let likeDetails = {};
        likeDetails['id'] = postId;
        if(likeDetails['id']) {
            this.likedPostInfo.push(likeDetails);
    ...
    render() {
        ...
        return (
            <React.Fragment>
                <Header searchHandler={this.searchCaptionHandler} userPicture={profile_picture} />
                        ...
                        return (
                            <Card className={classes.root} key={'imagePost' + imagePost.id}>
                                ...
                                <CardActions disableSpacing className={classes.likeAlignment}>
                                    ...
                                    <Typography variant="body2" className={classes.likeStyle}>{(imagePost['likes']['count'] > 0) && imagePost['likes']['count']} {(imagePost['likes']['count'] > 1) ? 'likes' : (imagePost['likes']['count'] > 0) && 'like'}</Typography>
                                </CardActions>
